...
public class FeeDueListSchoolWise 
{
	...
	public String execute() 
	{
		try 
		{
			...
			ps2= con.prepareStatement("select admn,name,place,duefrom,outstanding,transduefrom,admfees,annfees,mob,class,tutiondisc,transdisc from students order by class,name");
	...
	while(rs2.next())
	{
			...
			Outstanding = Outstanding + AdmFees;
			TotalTutionFees = TutionFees * TutionMonthBetween;
			TotalTransFees = TransFees * TransMonthBetween;
			TotalAmount = AnnFees+(TotalTutionFees)+(TotalTransFees)+Outstanding;
